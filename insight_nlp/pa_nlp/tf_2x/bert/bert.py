...
from pa_nlp.tf_2x.bert.open_source import bert
...
class Bert(tf.keras.layers.Layer):
  ...
  def call(self, input, token_type_ids, training: bool, *args, **kwargs):
    ...
    if token_type_ids is None:
      return self._bert(
        [input, tf.zeros_like(input)], mask=padding_mask, training=training,
      )
    else:
      return self._bert(
        [input, token_type_ids], mask=padding_mask, training=training
      )
